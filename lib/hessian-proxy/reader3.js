...
Reader.prototype.readReply = function(data) {
    ...
    var r = this.reader.nextString(1);
    assert(r === 'M', "Expect Reply start with 'M', but receive: '" + r + "'");
    return this.read();
};
...
Reader.prototype.readClassDef = function(data) {
    ...
    if (code === 't') {
        var clzDef = {};
        clzDef.name = this.readClassName();
...
Reader.prototype.readClassName = function(data){
    if (data)
        this.reader = new BufferReader(data);

    var len = (this.reader.nextUInt8()<<8)+this.reader.nextUInt8();
